...
void SequentialVMD::run(std::string videoPath) {
...
void SequentialVMD::benchmarkRun(std::string videoPath, int tries) {
    std::ofstream out;
...
void SequentialVMD::toGrayScale(cv::Mat& img) {
...
void SequentialVMD::smooth(cv::Mat& img) {
...
bool SequentialVMD::compareFrame(const cv::Mat& background, const cv::Mat& frame) {
    //TODO throw an error
